...
readableStream
    ...
    // bien acá por lo que busque y encontré.(porque el que busca encuentra) lo que hace util.inherits(Mayus,Transform); es crear una instancia de la clase Transform y estableciéndolo como prototipo a la función Mayus, tambien adjuntando el EventEmmitter. es decir el Transform.Call(this). de modo que cada vez que se crea una instancia de la funcion Mayus se ejecutara el fichero.
    ...
    // bueno bueno que tenemos acá. pero miren justo Mayus.prototype._transform fue posible gracias a la función de util.inherits(Mayus,Transform) que establecía la clase Transform como prototipo de la función Mayus.
    
    // PERO QUE SIGNIFICA ._transform ? bueno básicamente significa que la transformación que tu harás podrá ser personalizada, así es(como mi redacción). es decir que tu veras que cambio o que transformación le harás a la secuencia de datos que estas recibiendo como entrada.
    ...
    // bueno y por ultimo que sucede al final. pues simple creas una instancia de la funcion Mayus
